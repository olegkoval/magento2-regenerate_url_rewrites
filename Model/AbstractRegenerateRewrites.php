...
abstract class AbstractRegenerateRewrites
{
    ...
    protected function _prepareUrlRewrites(array $urlRewrites, string $urlSegmentToAppend = ''): array
    {
        ...
        $reservedPaths = [];
        // "store|path Magento generated" => path it was actually saved under (clean-up, SKU segment, dedup)
        $finalPaths = [];
        ...
        // as it is (no SKU segment, no sanitizing, no dedup index), or the URL they exist for is lost
        ...
        // actually saved, or the redirect lands on a URL that no longer exists (e.g. with --add-sku-to-url)
